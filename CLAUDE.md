...
- `crates/atlas-testkit`: dev-only. The harness the `atlasd`, `atlas-cli` and `atlas-core` tests share: daemon boots, the fixture repository, MCP round trips, stub models and the plugin channel stand-in. Each crate's `tests/common/mod.rs` binds it to that crate's own binary.
- `crates/atlas-client`: library holding `daemon_ctl` (start, stop and reach the daemon) and `remote` (the `RemoteBackend` HTTP client for the `Backend` trait), shared by `atlas-cli` (re-exported from its `lib.rs`) and the desktop app so the app does not pull in `atlas-cli`'s clap, ratatui and rmcp dependencies.
