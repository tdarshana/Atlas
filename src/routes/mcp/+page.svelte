<script lang="ts">
	// The MCP view: what used to be the Settings screen's MCP server card, now a full
	// rail destination. Same data (`GET /api/v1/mcp/status`), same actions (copy both
	// connect snippets, restart the daemon, toggle a tool globally); Settings itself now
	// only summarises it with a link here.
	...
	// A side panel row's jump lands here as a hash; scroll to it once the report has
	// loaded and the section exists to scroll to.
	$effect(() => {
		const id = page.url.hash.replace(/^#/, '');
		if (!id || !mcp.report) return;
		document.getElementById(id)?.scrollIntoView({ behavior: 'instant', block: 'start' });
	});

	$effect(() => {
		setStatusItems({ right: [{ text: countsText }] });
	});
</script>
...
<div class="title-row">
	...
	<Button variant="ghost" size="sm" data-testid="mcp-copy-claude" onclick={() => copy('claude', CLAUDE_SNIPPET)}>
		{copied === 'claude' ? 'Copied' : 'Copy Claude command'}
	</Button>
	<Button variant="ghost" size="sm" data-testid="mcp-copy-codex" onclick={() => copy('codex', CODEX_SNIPPET)}>
...
<div class="pane" data-testid="mcp-page">
	...
	<div class="group" id="clients">
		...
		<div class="clients-table" data-testid="mcp-clients">
			<Table id="mcp-clients" columns={clientColumns} rows={mcp.report?.clients ?? []} rowKey={(c: McpClient) => c.id}>
		...
		<span class="hint">
			A stdio client's call count refreshes on its 60 s heartbeat, so it can lag behind the
			calls it has actually made.
		</span>
	</div>
	...
	<div class="group" id="tools">
		...
		<div class="tools-table" data-testid="mcp-tools">
			<Table id="mcp-tools" columns={toolColumns} rows={mcp.report?.tools ?? []} rowKey={(t: McpToolRow) => t.name}>
