<script lang="ts">
	// The Agents view: the library of agents (roles) a project can put on its roster. Search
...
<div class="title-row">
	...
	/>
	<Button size="sm" data-testid="persona-new" onclick={startNew}>New agent</Button>
</div>
...
<div class="pane" data-testid="personas-page">
	...
	<div class="split">
		{#if !personas.loading && personas.items.length === 0 && !personas.error}
			<EmptyState title="No agents yet" hint="An agent is a role a project can put on its roster: its skills, workflows, practices, MCP servers, models and access.">
				<Button size="sm" onclick={startNew}>New agent</Button>
			</EmptyState>
		{:else}
...
<!-- An agent not yet created is drafted in a modal rather than the docked panel; once
...
{#snippet panel()}
	<aside class="detail" aria-label="Agent detail" data-testid="persona-detail">
		<header>
...
{#snippet form()}
	{#if draft}
		...
		<div class="actions full">
			...
			{#if draft.id !== null}
				{#if confirming}
					<span class="hint">Delete this agent?</span>
					<Button
						size="sm"
						variant="danger"
						disabled={saving}
...
<!-- Writing the roster into Claude Code and Codex lives with the roles themselves. -->
<SyncPanel checkOnMount={page.url.searchParams.get('sync') === '1'} />
