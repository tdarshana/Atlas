...
A project opens as a hub of eight tabs over one root: Profile, Board, Memories, Practices, Agents, Workflows, Log and Project settings. The board's lanes resize and remember their widths, the open task docks beside them, and the Log tab merges task events, memory writes, sync rows and extraction jobs into one history you can filter and export as JSONL. Project settings holds the board key prefix, an agent access rule saying which agents may write memories or move tasks here, and an extraction override that falls back to the global settings field by field. The same three are reachable from the CLI through `atlas project set` and `atlas project log`. See [Project hub](docs/features/phase8-project-hub.md) for the tabs and [Project settings](docs/usage.md#project-settings) in `docs/usage.md` for the rules, including why agent access is coordination rather than security.
