...
Any MCP client that speaks streamable HTTP can use `http://127.0.0.1:7433/mcp` once `atlas daemon start` has run. Atlas serves 29 tools, plus resources (agents, practices, workflows, project context and boards, recent memories) and prompts (`atlas.bootstrap`, `atlas.handoff`, `atlas.board_workflow`, and one per saved agent); `mcp.disabled_tools` can turn any tool off (`project_connect` and `memory_review` are off by default), and `GET /api/v1/mcp/status` reports the live tool table alongside who is connected over stdio or HTTP. The desktop app's Settings screen has an MCP server card for all of this: the connect snippets, the tools table with its enable checkboxes, connected clients, and a `Restart` button. See [MCP tools](docs/usage.md#mcp-tools) in `docs/usage.md` for the full tool table, the resources, the prompts and the status route.
...
A project opens as a hub of nine tabs over one root: Profile, Board, Memories, Practices, Agents, Workflows, Frameworks, Log and Project settings. The board's lanes resize and remember their widths, the open task docks beside them, and the Log tab merges task events, memory writes, sync rows and extraction jobs into one history you can filter and export as JSONL. The Frameworks tab shows what Atlas detected among Superpowers, OpenSpec, SpecKit and GSD, previews their documents, and imports their tasks and decisions onto the board and into pending memories; a task imported this way shows its source and a Re-import action in its own detail panel. Project settings holds the board key prefix, an agent access rule saying which agents may write memories or move tasks here, and an extraction override that falls back to the global settings field by field. The same three are reachable from the CLI through `atlas project set` and `atlas project log`. See [Project hub](docs/features/phase8-project-hub.md) for the tabs, [Framework compatibility](docs/features/phase12-frameworks.md) for the Frameworks tab, and [Project settings](docs/usage.md#project-settings) in `docs/usage.md` for the rules, including why agent access is coordination rather than security.
