...
`atlas sync --check` reports what would change without writing, and exits 1 if anything would. `atlas sync --global` writes the same agent files into your home directory instead of a project, or into `--home <path>` when you name one. A project sync only writes into a git repository.
